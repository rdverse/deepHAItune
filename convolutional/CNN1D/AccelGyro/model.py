...
def build_model_CNN(hp):
    ...
    inputA = layers.Input(shape=(50, 3))
    modelA = inputA

    for i in range(hp.Int(name='num_layers_A', min_value=1, max_value=2)):
        modelA = layers.Conv1D(filters=hp.Int(name='layer{}_CNN_A'.format(i),
                                              min_value=min_conv1,
                                              max_value=max_conv1,
                                              step=9,
                                              default=27),
                               padding='same',
                               activation='relu',
                               kernel_size=hp.Int(name='k{}_size_A'.format(i),
                                                  min_value=ksize_min,
                                                  max_value=ksize_max,
                                                  step=3,
                                                  default=3))(modelA)
    ...
    inputG = layers.Input(shape=(50, 3))
    modelG = inputG

    for i in range(hp.Int(name='num_layers_G', min_value=1, max_value=2)):
        modelG = layers.Conv1D(filters=hp.Int(name='layer{}_CNN_G'.format(i),
                                              min_value=min_conv1,
                                              max_value=max_conv1,
                                              step=9,
                                              default=27),
                               kernel_size=hp.Int(name='k{}_size_G'.format(i),
                                                  min_value=ksize_min,
                                                  max_value=ksize_max,
                                                  step=3,
                                                  default=3),
