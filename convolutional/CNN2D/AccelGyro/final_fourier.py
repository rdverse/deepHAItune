...
def build_model_CNN():
    inputA = layers.Input(shape=(3, data_attr[0], 1))
    modelA = inputA
    min_conv1 = 27
    max_conv1 = 36
    min_conv2 = 36
    ...
    modelA = layers.Lambda(lambda v: tf.cast(
        tf.signal.fft2d(tf.cast(v, dtype=tf.complex64)), tf.float32))(modelA)
    ...
    inputG = layers.Input(shape=(3, data_attr[0], 1))

    modelG = inputG

    modelG = layers.Lambda(lambda v: tf.cast(
        tf.signal.fft2d(tf.cast(v, dtype=tf.complex64)), tf.float32))(modelG)
