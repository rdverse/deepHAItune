...
def build_model_CNN(hp):
    ...
    num_lstm_layers = range(
        hp.Int(name='num_layers_A', min_value=1, max_value=2))
    print(len(num_lstm_layers))

    for i in num_lstm_layers:
        modelA = layers.Conv2D(filters=hp.Int(name='layer{}_LSTM_A'.format(i),
                                              min_value=min_conv1,
                                              max_value=max_conv1,
                                              step=9,
                                              default=27),
